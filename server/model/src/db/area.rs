    ...
    fn to_feature(self, mode: String) -> Result<Feature, ModelError> {
        ...
        if mode != Type::Unset {
            let coords = match mode {
                ...
                Type::CircleStation => self.fort_mode_route,
                _ => None,
            };
...
impl Query {
    ...
    async fn upsert_feature(
        ...
        if let Some(name) = name {
            ...
            if let Some(column) = column {
                ...
                let is_update = area::Entity::find()
                    .select_only()
                    .column(area::Column::Id)
                    .column(area::Column::Name)
                    .filter(area::Column::Name.eq(name.clone()))
                    .into_model::<NameId>()
                    .one(conn)
                    .await?;

                if let Some(existing) = is_update {
                    area::Entity::update_many()
                        .col_expr(column, Expr::value(area))
                        .filter(area::Column::Id.eq(existing.id.to_owned()))
                        .exec(conn)
                        .await?;
                    ...
                    area::Entity::insert(new_model).exec(conn).await?;
    ...
    pub async fn upsert_from_geometry(
        ...
        Ok((insert_update.inserts, insert_update.updates))
    }
}
