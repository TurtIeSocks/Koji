...
/// Parse the raw request body into [`LegacyArgs`], surfacing a 400 on malformed
/// JSON.
fn parse_args(request_json: &JsonValue) -> Result<LegacyArgs, HttpResponse> {
...
/// Serialize the [`LegacyArgs`] into the tagged [`CalcRequest`] JSON the worker
/// re-decodes. `instance` is overwritten with the resolved display name first so
/// the worker labels features with it (mirrors the old `request_json["instance"]`
/// injection). The flat v1 wire is unchanged; only the internal payload is typed.
fn legacy_request_json(
    mut legacy: LegacyArgs,
    mode: &str,
    instance: &str,
) -> Result<JsonValue, Error> {
    legacy.instance = Some(instance.to_string());
    serde_json::to_value(legacy.into_calc_request(mode))
...
async fn bootstrap(
    ...
    let legacy = match parse_args(&request_json) {
        Ok(args) => args,
    ...
    let LegacyResolved {
        ...
        parent,
        ..
    } = legacy.clone().init(Some("bootstrap"));
    ...
    let request = legacy_request_json(legacy, "bootstrap", &instance)?;
...
async fn cluster(
    ...
    let instance = resolve_instance(&conn, instance, &parent).await?;
    let request = legacy_request_json(legacy, &mode, &instance)?;
...
async fn reroute(
    ...
    let LegacyResolved {
        ...
        data_points,
        ..
    } = legacy.clone().init(Some("reroute"));

    let request = legacy_request_json(legacy, "reroute", &instance)?;
...
async fn route_stats(
    ...
    let LegacyResolved {
        ...
        instance,
        ..
    } = legacy.clone().init(Some("route-stats"));
    ...
    let request = legacy_request_json(legacy, "route-stats", &instance)?;
...
#[post("/area")]
async fn calculate_area(payload: web::Json<LegacyArgs>) -> Result<HttpResponse, Error> {
    let LegacyResolved { area, .. } = payload.into_inner().init(Some("calculate_area"));
