...
| `model/db/{gym,pokestop,spawnpoint,station}` | **R→koji-scanner** | read-only golbat access | #11 isolate scanner | High |
...
| `model/lib.rs` `ScannerType`/`KojiDb` | **RW** | RDM/Hybrid branching; 3-conn struct | #5 #6; →2-conn | High |
| `model/utils/get_database_struct` | **RW→koji-service config** | scanner-type autodetect + deprecated env | #5 #6 managed state | High |
