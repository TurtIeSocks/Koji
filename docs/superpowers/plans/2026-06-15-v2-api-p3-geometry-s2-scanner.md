# v2 API — Phase 3: Geometry transforms / S2 / scanner-data — Implementation Plan
...
**Goal:** Regroup the `/api/v2/geo/*` grab-bag into honest namespaces and re-nest scanner-data under its geofence:
...
- `/api/v2/geofences/{id}/scanner-data?category=&lastSeen=` (was `/scanner-data/{category}?instance=`) — id-consistent sub-resource.

**Architecture:** `geo.rs` splits into `geometry.rs` (the three transforms) + `s2.rs` (the four S2 ops), each with a `scope()`; `geo.rs` is deleted. The transforms already deserialize typed requests (`ConvertReq`/`SimplifyReq`/`MergePointsReq` in `requests/ops.rs`) — they just need `respond_geo` + `ServiceError`. scanner-data moves into the geofences scope, keyed by the geofence path id (id/name) instead of an `?instance=` query.
...
- **scanner-data `lastSeen` query is camelCase** (a config-style query param, like calc args), defaulting to 0; `category` is a query param. The geofence `{id}` (id or name) supplies the area.
...
## Task 3: scanner-data → geofence sub-resource

**Files:** Modify `crates/koji-service/src/public/v2/scanner_data.rs`. Read it first.

- [ ] **Step 1: Re-key the handler on the geofence path id**

New shape: `GET /api/v2/geofences/{id}/scanner-data?category=&lastSeen=`.
...
git commit -m "feat(koji-service): scanner-data → /geofences/{id}/scanner-data (id-keyed), ServiceError"
```
...
**Files:** Modify `crates/koji-service/src/lib.rs`; possibly `public/v2/geofences.rs` (to host the scanner-data sub-route).
...
- remove `.service(public::v2::scanner_data::scanner_data)` (the old flat mount).
- mount scanner-data under geofences: in `geofences::scope()` add `.service(web::resource("/{id}/scanner-data").route(web::get().to(crate::public::v2::scanner_data::scanner_data)))` (alongside `/{id}` and `/{id}/publish`). Confirm the `scanner_data` handler is `pub(crate)` and reachable.
...
git commit -m "feat(koji-service): rewire /v2 — /geometry + /s2 scopes, scanner-data under geofences"
```
...
rg -n "geometry::scope|s2::scope|scanner-data" crates/koji-service/src/lib.rs crates/koji-service/src/public/v2/geofences.rs
```
Expected: `CLEAN`; the new `/geometry`, `/s2`, and `/{id}/scanner-data` registrations present.

- [ ] **Step 3: DB-run note**

Live smoke (real DB): `POST /api/v2/geometry/convert` (GeoJSON in envelope; `?format=sql` raw); `POST /api/v2/s2/15` (cells); `GET /api/v2/geofences/{id}/scanner-data?category=pokestop&lastSeen=0` (points; 404 unknown fence; 400 bad category). Recorded in the spec's P3 testing notes.

---

## Self-Review

**Spec coverage:** `/geometry/*` regroup (§4.3, §A12) ✓ · `/s2/*` lifted (§4.4, §A12) ✓ · scanner-data sub-resource (§4.5, §A11) ✓ · transforms envelope/`?format` (§6.4) ✓ · `ServiceError` (§6.2) ✓. `/geo/*` deleted.
