# Koji V2 — Phase 1c: split `model` → `koji-db` + `koji-golbat` (design)
...
- **`koji-golbat`** — read-only access to golbat's data DB (`gym`, `pokestop`, `spawnpoint`, `station`).
...
Split `model::utils` (pure → `koji-core`, db → `koji-db`, golbat-normalizers → `koji-golbat`). Migrate consumers (`api`, `algorithms`) as types move — **no transitional facades**.
...
- `model::api::text` (`TextHelpers::parse_golbat_instance`) needs koji-db's **RDM types** (`RdmInstance`, `InstanceParsing`).
...
```
koji-core  ← koji-golbat, koji-db, model, algorithms, api      (universal sink)
koji-db    ← model, api                                          (→ koji-core)
koji-golbat ← api                                               (→ koji-core)
model      ← api                                                 (→ koji-core, koji-db; api/args + api/text-less remainder)
algorithms ← api                                                 (→ koji-core; drops its db-enum dep, see §enums)
```
No cycles. `koji-golbat` and `koji-db` are independent siblings (no koji-db↔koji-golbat edge — confirmed: no koji-table query reads a golbat table, and golbat result types aren't used by koji-db queries).
...
### `koji-golbat` (new; → koji-core)
- Entities + queries: `gym`, `pokestop`, `spawnpoint`, `station` (all read the `golbat` connection).
- Golbat result/row types: `LatLonRow`, `Spawnpoint`, `GenericData`, `GenericDataToVec`.
- Golbat normalizers (from `model::utils::normalize`): `fort`, `fort_filtered`, `spawnpoint`, `spawnpoint_filtered`.
- **`GolbatError`** (own thiserror error; wraps `DbErr`/`geojson::Error`). Pre-stages P7's per-crate error types and keeps `koji-golbat → koji-core` only.
- Queries take `&DatabaseConnection` (the caller passes `KojiDb.golbat`); the crate does not own connections.
...
- Connection layer: **`KojiDb`** struct, `GolbatType`, `get_database_struct` (RDM detection stays until P6).
...
- `lib.rs`: loses `KojiDb`/`GolbatType` (→ koji-db). Becomes a thin `pub mod api;`.
...
- `api` crate: `model::db::{gym,pokestop,spawnpoint,station}` → `koji_golbat::…`; `model::db::{area,geofence,instance,project,route,…}` → `koji_db::…`; `model::{KojiDb,GolbatType}` → `koji_db::…`; `model::error::ModelError` → `koji_db::ModelError`; `model::api::{GeoFormats, args::{ApiQueryArgs,BoundsArg,…}}` → `koji_core::…`; calc `Args`/`ReturnTypeArg` stay `model::api::args::…`.
...
## Workspace / naming
- New members `crates/koji-db`, `crates/koji-golbat` in `[workspace.members]`; deps via `{ workspace = true }`.
...
2. **koji-golbat** — new crate; move golbat entities + result types + normalizers + `GolbatError`; re-point `api` golbat imports. Green.
...
2. **`koji-golbat` gets its own `GolbatError`; `koji-db` keeps `ModelError`.** Avoids `koji-golbat → koji-db` and pre-stages P7 per-crate errors. (Alternative considered: one shared error in koji-db — rejected as it couples golbat to Koji's db.)
