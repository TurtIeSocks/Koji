...
- Activate
  ...
  - This is currently a "hack" to save on performance when multiple
    Distance Colors
...
- Category
  - Selec the category/table for the clustering algorithm to read from the database
- Strategy
...
- Fast
  - Uses an exponentially faster algorithm but returns worse results
- Only unique
  - 🚧 Experimental 🚧
  - Counts min points by unique points instead of all points in a Cluster
