...
We start by setting up the mutable variables that help us keep track of where things are during the `while` loop runs.
...
We iterate through `local_clusters` in serial and push the best cluster to the `new_clusters` HashSet.
