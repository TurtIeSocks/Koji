...
The radius of each cluster to encase the `data_points` within
...
This groups `data_points` based on their S2 cell level before clustering them. The groups are then ran on separate threads in order to help with parallelizing workloads.
