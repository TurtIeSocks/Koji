...
While Kōji does connect directly to your scanner's database, most scanners have their geofences or routes loaded into memory and must reload these in order to make use of the latest changes. Instead of restarting your scanner process, you can set up Kōji to call the refresh endpoint automatically whenever a geofence or route is updated in the scanner database.

<Steps>

#### In the Admin Panel, navigate to the `projects` page.

#### Edit (or create) a scanner project.

#### Set `scanner` to `true`.

#### Add the `api_endpoint` value. A hint is provided for both RDM and Dragonite scanner endpoints.

#### Add the `api_key` value. A hint is provided for both RDM and Dragonite scanner endpoints.

</Steps>

Now when you click the `Sync` button, either next to the respective project, geofence, or route, Kōji will automatically call this endpoint after saving the changes to the scanner database, making them immediately available to the scanner.
