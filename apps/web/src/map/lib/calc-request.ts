...
export function featureToAreaFC(
...
export function routeCoordsToClusters(
...
export interface CalcInputs {
	...
	category?: string;
	/** "Last seen after" filter as epoch SECONDS (`dataFilter.lastSeen`) — the
	 *  server only clusters points `updated >` this instant, matching the map's
	 *  marker preview (v1 sent this too; omitting it clustered stale points the
	 *  picker hides). `0`/absent = no filter. Cluster/route mode only — the
	 *  backend gives bootstrap/reroute/routeStats a fixed no-filter. */
	lastSeen?: number;
}

/** Build the POST /api/v2/jobs body from the panel params + resolved inputs.
 *  Field names are the v2 backend wire names (camelCase). */
export function buildCalcBody(
	p: CalcParams,
	inp: CalcInputs,
	...
	if (p.strategy === "s2") {
		...
		if (p.maxClusters && p.maxClusters > 0)
			clustering.maxClusters = p.maxClusters;
		if (p.centerClusters) clustering.centerClusters = true;
	}
	const filter: Record<string, unknown> = {};
	if (p.tth && p.tth !== "All") filter.tth = p.tth;
	if (inp.lastSeen && inp.lastSeen > 0) filter.lastSeen = inp.lastSeen;
	const dataFilter =
		Object.keys(filter).length > 0 ? { dataFilter: filter } : {};
	return {
		mode: "route",
		category: inp.category,
		area: inp.area,
...
export function buildRouteStatsBody(
	inp: RouteStatsInputs,
): Record<string, unknown> {
	return {
		mode: "routeStats",
...
export function parseCalcResult(
	...
	return {
		fc: isFC ? (data as GeoJSON.FeatureCollection) : null,
		stats: record?.result?.stats ?? null,
	};
}
