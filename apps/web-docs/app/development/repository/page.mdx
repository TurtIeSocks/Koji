...
<FileTree>
  <FileTree.Folder name="server" defaultOpen>
    ...
    <FileTree.Folder name="model">
      <FileTree.File name="DB models, query layer, golbat adapters" />
    </FileTree.Folder>
  ...
  <FileTree.File name="Dockerfile (multi-stage client + server build)" />
...
- `server/model`: SeaORM models/query helpers and golbat-specific data access.
