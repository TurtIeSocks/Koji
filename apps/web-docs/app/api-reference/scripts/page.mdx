...
For UI/internal session endpoints (`/config/*` and `/internal/*`), see [Internal and Config API](./internal).
...
## cURL: Save Geofences to Golbat

```bash
curl -sS -X POST \
  -H "Authorization: Bearer KOJI_SECRET" \
  -H "Content-Type: application/json" \
  "http://127.0.0.1:8080/api/v1/geofence/save-golbat" \
  -d '{
    "area": {
...
## JavaScript: Sync All Project Geofences to Golbat
