...
Since all routes are required to be associated with a geofence, Kōji has a special built-in function to keep them in sync as painlessly as possible.

If the routes associated with a geofence share the same name as that geofence, their name will be updated whenever the geofence's name is updated. This is a great way to keep your routes organized and synced up with the golbat.
...
While Kōji does connect directly to your golbat's database, most golbats have their geofences or routes loaded into memory and must reload these in order to make use of the latest changes. Instead of restarting your golbat process, you can set up Kōji to call the refresh endpoint automatically whenever a geofence or route is updated in the golbat database.

<Steps>

#### In the Admin Panel, navigate to the `projects` page.

#### Edit (or create) a golbat project.

#### Set `golbat` to `true`.

#### Add the `api_endpoint` value. A hint is provided for both RDM and Dragonite golbat endpoints.

#### Add the `api_key` value. A hint is provided for both RDM and Dragonite golbat endpoints.

</Steps>

Now when you click the `Sync` button, either next to the respective project, geofence, or route, Kōji will automatically call this endpoint after saving the changes to the golbat database, making them immediately available to the golbat.

### Default Templates

Since most golbats have many other properties associated with their geofences and routes, Kōji provides a way to automatically add these properties to your geofences and routes when they are created.
...
<Table>
  ...
  <tbody>
    <Table.Tr>
      <Table.Td>auto_quest</Table.Td>
      <Table.Td>AQ</Table.Td>
    </Table.Tr>
    <Table.Tr>
      <Table.Td>circle_pokemon</Table.Td>
      <Table.Td>CP</Table.Td>
    </Table.Tr>
    <Table.Tr>
      <Table.Td>circle_smart_pokemon</Table.Td>
      <Table.Td>CSP</Table.Td>
    </Table.Tr>
    <Table.Tr>
      <Table.Td>circle_raid</Table.Td>
      <Table.Td>CR</Table.Td>
    </Table.Tr>
    <Table.Tr>
      <Table.Td>circle_smart_raid</Table.Td>
      <Table.Td>CSR</Table.Td>
    </Table.Tr>
    <Table.Tr>
      <Table.Td>pokemon_iv</Table.Td>
      <Table.Td>IV</Table.Td>
    </Table.Tr>
    <Table.Tr>
      <Table.Td>leveling</Table.Td>
      <Table.Td>L</Table.Td>
    </Table.Tr>
    <Table.Tr>
      <Table.Td>auto_tth</Table.Td>
      <Table.Td>ATTH</Table.Td>
    </Table.Tr>
    <Table.Tr>
      <Table.Td>auto_pokemon</Table.Td>
      <Table.Td>AP</Table.Td>
    </Table.Tr>
  </tbody>
</Table>
