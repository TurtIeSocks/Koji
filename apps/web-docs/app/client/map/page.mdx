...
- Mode
  ...
  - `Route` - clusters and routes the points using tsp-mt (native Rust)
- Category
...
Below are three community–tuned example configurations you can apply under **Calculations** to get up and running quickly. Feel free to tweak them for your golbat density or area size.
...
- 🚧 Save to Golbat Db 🚧
  - Saves the route to the Golbat database, use with Calculation
- Skip rendering
...
- Import from Golbat
  - Lets you load in routes and geofences from the golbat database to preview in Kōji
    <Callout type="info" emoji="💡">
      It's recommended to import your fences and routes from the golbat
      database via the Import Wizard, then only use this import selector to
      preview what your golbat is currently using. You should only load and
