...
> **NEXT — P7 (final).** koji-cli (run|enqueue|worker) + koji-plugins extraction (deferred P2 move + manifest/JSON protocol/registry; move `create_cell_map`/`stringify_points`) + **v1-calc re-point through the sync bridge** + rename `api`→`koji-service` + thiserror/clippy-pedantic/fmt sweep + OpenAPI + open PR. **Client (React) follow-up:** `client/src` still reads `res.scanner_type` + branches on `'unown'` (~10 files) — collapse to the single path (the config response no longer carries `scanner_type`).
