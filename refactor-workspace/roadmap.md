...
> **Done + committed:** … P2-configs (`c138d0b`) · P3 (`fb25e8e`→`a42bf18`) · **P4-core (`a4d7041`)** — AppState (db+JobQueue+workers+EventDispatcher+optional DragoniteClient) + JSend + `CalculateHandler` (calc-as-job) + v2 `/jobs` + `/calc/*` sync-bridge + `/meta/algorithms` + `/healthz`; v1 untouched. (Maintainer steer: clean v2, v1 byte-compat moot.)
> **NEXT:** **finish P4** — v2 typed CRUD resources (geofences/routes/projects/properties/tile-servers, JSend + `?format=`, reuse koji-db queries, macro the shape) + rename `api`→`koji-service`. Then **P5** (events→Dragonite: wire the deferred calc-persist/publish to `events.publish` + `DragoniteSubscriber`; needs the verified queue + reconciled Dragonite client). **P5/P6 need maintainer runtime verification** (DB + live Dragonite) — P6 (RDM removal) is destructive + ordering-gated behind P5.
