...
> **▶ RESUME HERE (paused 2026-05-28).** Branch `claude/goofy-tu-5372ed`; tree clean, `cargo build --workspace` + tests green (16+3 pass, 4+1 ignored).
> **Done + committed:** P0 (`410fe72`, `a1120c0`) · P1a (`77fb53e`) · P1b (`0e555b3`).
> **NEXT:** execute the approved plan `docs/superpowers/plans/2026-05-28-koji-v2-p1conv-conversion-modernization.md` via `superpowers:executing-plans` — it's planned but **not run**. Then plan P1c.
...
- **P1-conv** 📋 PLANNED, ready to execute (`docs/superpowers/plans/2026-05-28-koji-v2-p1conv-conversion-modernization.md`) — conversion-layer modernization: `FeatureCtx` for the param-carrying conversions + a `wrapper_conversions!` derive macro + `From` where the target is local. (Orphan rule keeps the geometry aliases on `To*` traits.)
