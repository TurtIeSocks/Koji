...
> **P7 IN PROGRESS (quality + rename + CLI landed).** ✅ clippy deny-level cleared (`5923957`: greedy.rs `unused_io_amount`→`write_all`) · ✅ rename `api`→`koji-service` (`04a9af0`) · ✅ **koji-cli** (`08831b5`: `worker|enqueue|get` over the job queue) · ✅ clippy `--fix` on koji-service (`ea9efdd`). Full suite green; final runtime smoke on the latest binary green (healthz/v1-calc/v2-calc/auth/publish→PATCH).
> **Remaining P7 (each a fresh, substantial unit — explicit follow-ups, not blockers):**
>   1. **koji-plugins extraction + formalization** (manifest/JSON protocol/registry; move `create_cell_map`→koji-core, `stringify_points`→koji-plugins to break the algorithms↔plugins cycle). **Design-heavy** — warrants its own brainstorm/spec (open protocol choices), like the v1-byte-compat gate.
>   2. **OpenAPI doc** — recommend `utoipa` derive-from-handlers over a hand-written spec (avoids drift); deferred.
>   3. **clippy-pedantic sweep** — residual ~manual warnings (large-variant boxing, manual trait impls) across new+legacy crates; low-value style churn, deferred.
>   4. **repo-wide rustfmt** — never enforced → huge diff; do as ONE isolated final commit.
>   5. **client (React) collapse** — `client/src` (~10 files) still reads `res.scanner_type`/branches on `'unown'`; separate frontend effort (config response no longer carries it).
>   6. **v1-calc re-point through the bridge** — DEFERRED on purpose: v1 is deprecated (Dragonite→v2), so re-pointing a working legacy surface through the queue is low-value + non-zero risk. Leave v1 calc inline.
> **▶ PR is READY pending the maintainer's go-ahead** (CLAUDE.md gates PR-opening behind a heads-up). Branch `claude/goofy-tu-5372ed` holds the complete, green, runtime-verified P0–P6 migration + P7 quality/rename/CLI.
