...
> **P7 ✅ COMPLETE.** ✅ rename `api`→`koji-service` (`04a9af0`) · ✅ **koji-cli** (`08831b5`) · ✅ **koji-plugins** extraction + formalization (`ae57081`+`04fe12c`+`4df9a72`; manifest/JSON-protocol/registry/`KOJI_PLUGINS_DIR`, `create_cell_map`→koji-core, acyclic) · ✅ **OpenAPI 3.1** at `GET /api/v2/openapi.yaml` (`829229d`) · ✅ clippy deny-fix (`5923957`) + workspace `--fix` (`8679bb6`,`ea9efdd`) · ✅ **rustfmt --all** (`551a3ab`) · ✅ **React client `scanner_type` collapse** (`cf1f0ff`) · ✅ **v1-calc re-point through the queue** (`a7d21a8`) — all v1 calc now queue-backed (single pipeline), legacy envelope preserved; `/area` stays inline · ✅ **fix(jobs,events): `notify_one` heartbeat-stop** (`e7fb172`) — a pre-existing fast-job race (lossy `notify_waiters` → worker wedged, job stuck `running`) surfaced by the near-instant reroute/route-stats jobs; also fixes fast v2 calc. `cargo test --workspace` + `cargo clippy` (deny-level 0) + `cargo fmt --check` green; runtime-verified (all v1+v2 calc/CRUD, 5 rapid reroutes, publish→PATCH).
> **Deferred follow-ups (documented, not blockers):** `area.route_updated` producer (subscriber arm + mapping exist, unit-tested; needs a koji-mode→`AreaMode` decision) · calc `save_to_db`/`save_to_scanner` v2 side-effects (deferred since P4) · residual manual clippy-pedantic warnings (large-variant boxing / manual trait impls — low-value churn).
> **Post-audit hardening (after a self-audit):** ✅ **`tsp` route-optimizer regression fixed** (`c53cda8`) — P7 plugins formalization broke the Docker-built OR-Tools `tsp` router; added a `protocol="latlng"` manifest option + `plugins/tsp/plugin.toml` + `KOJI_PLUGINS_DIR`; E2E-verified · ✅ **`area.route_updated` producer** (`79ff475`) — `POST /api/v2/routes/{id}/publish` (route→geofence→`dragonite_area_id` + `mode→AreaMode` map); E2E-verified PATCH `quest_mode.route` · ✅ heartbeat-race regression test (`0f93999`) + registry caching (`003b474`) + OpenAPI route-publish (`ded795b`). `cargo test --workspace` 96 passed / 0 failed; clippy deny-0; fmt clean.
> **Final deferred (maintainer-gated, documented):** confirm the route `mode→AreaMode` map; v2 calc `save_to_db` (async-model mismatch); prune dead `Type::CircleSmart*` (DB-enum data migration); manual clippy-pedantic; real-Dragonite test + perf-parity A/B + human review of subagent-authored work.
> **▶ ALL PHASES P0–P7 DONE + audit-hardened. PR is READY pending the maintainer's go-ahead** (CLAUDE.md gates PR-opening behind a heads-up). Branch `claude/goofy-tu-5372ed`; PR body in `refactor-workspace/PR-BODY.md`.
