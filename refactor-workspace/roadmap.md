...
> **NEXT.** **P6** (RDM removal) is now **unblocked** (P5 write-path replacement landed + verified) but remains **destructive**: per architecture §10, drop `ScannerType`/RDM/Hybrid, `instance.rs`, the `area.rs` controller-write paths, deprecated env fallbacks; `KojiDb` 3→2 conns (controller conn → Dragonite HTTP client). **P7**: koji-cli + koji-plugins extraction + thiserror/clippy/fmt sweep + v1-calc re-point through the bridge + rename `api`→`koji-service` + OpenAPI + open PR.
